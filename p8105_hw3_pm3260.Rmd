...
On first look, there are a LOT of missing data. This data set contains `r nrow(ny_noaa)` rows and `r ncol(ny_noaa)` variables (`r paste(names(ny_noaa), collapse = ", ")`).  The avg prcp number is `r mean(pull(ny_noaa, prcp), na.rm = TRUE)` and the avg snow is `r mean(pull(ny_noaa, snwd), na.rm = TRUE)`.

#### Data cleaning

```{r noaaclean, message= FALSE, warning= FALSE}
ny_noaa = 
  ny_noaa |>
...
The most observed value for snow is 0 and NA. This makes sense because it does not snow too much in NY.

```{r meannoaa,  message= FALSE, warning= FALSE}
ny_noaa |>
...
The temperature on avg is lower in Jan than July. The temperatures stay around a similar range within the two months, aside from a couple colder outliers (1983~ Jan, 1987~ July, 2004~ July). 

```{r noaaplot,  message= FALSE, warning= FALSE}
...
Most of the data is concentrated in the middle of the hex plot; there are also some data points (in the +,- quadrant) that indicate that some tmax values are less then tmin, which is concerning for the data. 
With the ridge plots, you can see clear peaks for all the years 0-35 mm, 45 mm, and 80 mm of snow, each smaller than the last. 
...
```{r acthr, message=FALSE, warning=FALSE}
activity_hr = 
  activity_df |>
...
```{r nyc_bikes, message=FALSE, warning=FALSE}
...
```{r citiplot, warning = FALSE, message = FALSE}
citi_plot = citi_df |>
  filter(year == 2024) |>
...
Member riders seem to all have similar distribution despite the type of bike or month, with short 10-20 min rides being the most common. Casual riders show a wider range of ride times. Ride duration seem to be longer in July than in January with a lightly wider distribution. For casual riders, when using electric bikes, their duration seems more concentrated to a shorter ride while classic bike tends to be more varied.
